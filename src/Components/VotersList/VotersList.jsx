...
const Voter = props => (
  <tr>
    ...
    <td>
      <Link to={"/edit/" + props.voter._id}>edit</Link> |{" "}
      <a
        href="/home"
        onClick={() => {
          props.deleteVoter(props.voter._id);
        }}
      >
        delete
      </a>
...
export default class VotersList extends Component {
  constructor(props) {
    ...
    this.voterList = this.voterList.bind(this);
  }

  componentDidMount() {
    axios
      .get("http://localhost:3001/api/voters/home")
      .then(response => {
  ...
  deleteVoter(id) {
    axios.delete("http://localhost:3001//api/voters/delete" + id).then(response => {
  ...
  voterList() {
    return this.state.voters.map(currentvoter => {
      return (
        <Voter
          voter={currentvoter}
          deleteVoter={this.deleteVoter}
          key={currentvoter._id}
        />
      );
    });
  }
